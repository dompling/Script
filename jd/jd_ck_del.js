...
try {
  if (condition) {
    ...
    $.log('当前删除条件')
    $.log(JSON.stringify(condition, null, `\t`))

    const delData = []
    const newData = cookies.filter((item, index) => {
      const where =
        condition.indexOf(`${index + 1}`) !== -1 ||
        condition.indexOf(item.userName) !== -1
      if (where) delData.push(item.userName)
      return !where
    })
    const newJDHelp = jdHelp.filter((item) => {
      return delData.indexOf(item.username) === -1
    })

    if (cookies.length !== newData.length) {
      $.log('=============删除信息=============')
      $.info(JSON.stringify(delData, null, `\t`))
      $.log('=================================')
