 ...
 *
 * 【示例】
 *  1.【input】: zh,enShort,enShort3,enFull （默认 zh）匹配规则，如果节点名称里面包含参数的规则，则会进行重命名
 *  2.【output】: zh,enShort,enShort3,enFull （默认 zh） 输出规则，如果节点名称与输入规则匹配，重命名之后为相关结果
 *  3.【prefix】: 重命名之后的前缀（可置空）
 *  4.【suffix】: 重命名之后的后缀（可置空）
 *
 * https://raw.githubusercontent.com/dompling/Script/master/sub.store/rename.js#input=zh&output=zh&prefix={前缀}&suffix={后缀}&del1&type=serif-bold&num=circle-regular
 */
...
// 获取前缀
const prefix =
  $arguments.prefix == undefined ? "" : decodeURI($arguments.prefix);

// 获取后缀
const suffix =
  $arguments.suffix == undefined ? "" : decodeURI($arguments.suffix);
...
// 获取机场名
const airport =
  $arguments.airport == undefined ? "" : decodeURI($arguments.airport);
